...
__global__ void addArrays(const int* a,const int* b, int* result, int size) {
    ...
    if (idx < size) {
        printf("Thread %d (Block %d, Thread in Block %d, Total Threads %d): Adding %d + %d = %d\n",
...
int main (){
    ...
    //LAunch the Kernel to add arrays on the device
    ...
    if(cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaDevicesSynchronize failed: %s\n", cudaGetErrorString(cudaStatus));
                cudaFree(deviceArray1);
                cudaFree(deviceArray2);
                cudaFree(deviceResultArray);
                exit(EXIT_FAILURE);
